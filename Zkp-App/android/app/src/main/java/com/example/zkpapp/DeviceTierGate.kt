...
object DeviceTierGate {
    ...
    suspend fun generateProof(
        ...
        try {
            ...
            val posted = postProofToCallback(callback, payload)
            if (!posted) {
                return@withContext DeviceTierResult.Error("Server POST failed")
            }

            Log.i(TAG, "✅ Tier 3 proof submitted successfully")
